...
exclamationDice(DiceType::EXCLAMATION)
    ...
    Game::~Game() 
    {
        ...
        for (auto location : locations) delete location;
    }
    ...
    void Game::start() 
    {
        ...
        while (!isGameWon() && !isGameLost()) 
        {
           ...
           displayGameStatus(*this , heroes[0],heroes[1]);
            ...
            // After each hero's turn, do monster phase
            monsterPhase();
    
            currentHeroIndex = (currentHeroIndex + 1) % heroes.size();
    
            // Only increment turn after both heroes have played
    ...
    void Game::initialize() 
    {
        ...
        // monsterPhase(); // Remove this line so monster phase does not run at game start
       // loadVillagers();
    ...
    void Game::loadItems() 
    {
        ...
        // Yellow Items
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Flower", ItemColor::Yellow, 2, "Docks"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Tarot Deck", ItemColor::Yellow, 3, "Camp"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Garlic", ItemColor::Yellow, 2, "INN"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("mirrored Box", ItemColor::Yellow, 3, "Mansion"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Stake", ItemColor::Yellow, 3, "Abbey"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Scroll of Thoth", ItemColor::Yellow, 4, "Museum"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Violin", ItemColor::Yellow, 3, "Camp"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Tablet", ItemColor::Yellow, 3, "Museum"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Wolfsbane", ItemColor::Yellow, 4, "Camp"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Charm", ItemColor::Yellow, 4, "Camp"));
    
        // Blue Items
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Anatomy Text", ItemColor::Blue, 1, "Institute"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Contrifuge", ItemColor::Blue, 1, "Laboratory"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Kite", ItemColor::Blue, 1, "Tower"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Research", ItemColor::Blue, 2, "Tower"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Telescope", ItemColor::Blue, 2, "Mansion"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Searchlight", ItemColor::Blue, 2, "Precinct"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Experiment", ItemColor::Blue, 2, "Laboratory"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Analysis", ItemColor::Blue, 2, "Institute"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Rotenone", ItemColor::Blue, 3, "Institute"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Cosmic Ray Diffuset", ItemColor::Blue, 3, "Tower"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Nebularium", ItemColor::Blue, 3, "Tower"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Monocane Mixture", ItemColor::Blue, 3, "INN"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Fossil", ItemColor::Blue, 3, "Camp"));
    
        // Red Items
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Dart", ItemColor::Red, 2, "INN"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Fire Poker", ItemColor::Red, 3, "Mansion"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Rapier", ItemColor::Red, 5, "Theatre"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Shovel", ItemColor::Red, 2, "Graveyard"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Torch", ItemColor::Red, 5, "Barn"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Pitchfork", ItemColor::Red, 4, "Barn"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Rifle", ItemColor::Red, 6, "Barn"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Silver Cane", ItemColor::Red, 6, "Shop"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Knife", ItemColor::Red, 3, "Docks"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Pistol", ItemColor::Red, 6, "Precinct"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Bear Trap", ItemColor::Red, 4, "Shop"));
        for (int i = 0; i < 2; ++i) allItems.push_back(Item("Speargun", ItemColor::Red, 4, "Institute"));
    
        // Ensure we have exactly 70 items. (There are 35 distinct items, each added twice to reach 70) 
        // If the count is not 70, there's an error in the loops above.
        if (allItems.size() != 70) 
        {
    ...
    void Game::loadLocations() 
    {
        ...
        // Set up location connections based on the new image
    ...
    {
        // Create monsters and place them in locations
        for (auto location : locations) 
        {
        ...
        frenzyMonster = monsters[0]; // Set Dracula as the frenzy monster
    }
    
    // void Game :: loadVillagers(){
    ...
    void Game::rewardHeroWithPerk(Hero* hero, Villager* villager) {
        villager->markSaved(); // ✅ علامت‌گذاری به عنوان نجات‌یافته
        villager->getLocation()->removeVillager(villager); // حذف از بازی
    ...
    void Game::assignHeroes() 
    {
        ...
        if (startingLocation) 
        {
            // Hero* hero1 = new Hero(playerName1, heroType1 == 1 ? HeroType::Archaeologist : HeroType::Mayor, startingLocation);
            // Hero* hero2 = new Hero(playerName2, heroType2 == 1 ? HeroType::Archaeologist : HeroType::Mayor, startingLocation);
            // heroes.push_back(hero1);
            // heroes.push_back(hero2);
            // startingLocation->addHero(hero1);
            // startingLocation->addHero(hero2);
    ...
    {
        int actions = 4;  // Each hero gets 4 actions per turn
        ...
        continue; // برگرد به منوی اصلی بدون کم‌شدن اکشن
    }
...
if (choice < 1 || choice > 10) {
    ...
    continue;
            ...
            switch (choice) {
                ...
                for (size_t i = 0; i < neighbors.size(); ++i) {
                    ...
                    if (false) { // No longer any specific transitional locations
                ...
                if (locItems.empty()) {
                    ...
                    for (size_t i = 0; i < locItems.size(); ++i) {
                        ...
                        std::getline(std::cin >> std::ws, lineInput); // Read the whole line
                      //  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                        ...
                        // Sort and remove duplicates to avoid issues with removal and ensure each item is processed once
                ...
                case 4: { 
                    ...
                    // حالت 1: هدایت Villager از مکان فعلی به خانه همسایه
                    ...
                    if (!localVillagers.empty()) {
                        ...
                        if (villagerChoice >= 1 && villagerChoice <= static_cast<int>(localVillagers.size())) {
                            ...
                            if (destChoice >= 1 && destChoice <= static_cast<int>(neighbors.size())) {
                                ...
                                villager->moveTo(dest); // :white_check_mark: بدون این، مکانش تغییر نمی‌کنه
                                ...
                                current->addVillager(villager); // بازگرداندن به خانه فعلی
                                villager->moveTo(current);
                            }
                        }
                    }
                
                    // حالت 2: هدایت Villager از خانه همسایه به مکان فعلی
                    const auto& neighbors = current->getNeighbors();
                    ...
                    for (Location* neighbor : neighbors) {
                        ...
                        if (!neighborVillagers.empty()) {
                            ...
                            if (villagerChoice >= 1 && villagerChoice <= static_cast<int>(neighborVillagers.size())) {
                                ...
                                villager->moveTo(current); // :white_check_mark:
                ...
                //         for (size_t i = 0; i < localVillagers.size(); ++i) {
                //             std::cout << i + 1 << ". " << localVillagers[i]->getName() 
                //                       << " (Safe place: " << localVillagers[i]->getDestination()->getName() << ")\n";
                //         }
                
                //         int choice;
                //         std::cout << "Choose villager to guide (0 to cancel): ";
                //         std::cin >> choice;
                
                //         if (choice > 0 && choice <= localVillagers.size()) {
                //             Villager* villager = localVillagers[choice - 1];
                //             Location* destination = villager->getDestination();
                
                //             // حرکت به مکان امن
                //             villager->moveTo(destination);
                //             current->removeVillager(villager);
                //             destination->addVillager(villager);
                
                //             // پاداش برای نجات
                //             if (!perkDeck.empty()) {
                //                 hero->addPerkCard(perkDeck.back());
                //                 std::cout << "🎉 " << villager->getName() << " saved! You got a Perk Card: " 
                //                           << perkDeck.back().getName() << "\n";
                //                 perkDeck.pop_back();
                //             }
                ...
                case 5: {  // Use a perk card
                    ...
                    if (perkCards.empty()) {
                        ...
                        perkCards[cardChoice - 1].activate(this, hero);  // ← استفاده از this چون داخل کلاس Game هستی
                
                        std::cout << "Used " << perkCards[cardChoice - 1].getName() << "\n";
                        hero->removePerkCard(cardChoice - 1);  // کارت رو پاک کن
                    ...
                    if (hero->getType() == HeroType::Archaeologist) {
                        ...
                        if (locItems.empty()) {
                            ...
                            if (chosenIndexes.empty()) {
                                ...
                                // Remove items in reverse order, checking index validity each time
                                for (int i = chosenIndexes.size() - 1; i >= 0; --i) {
                                    int indexToRemove = chosenIndexes[i];
    ...
    void Game::loadMonsterCards() {
        // Gray cards
        monsterDeck.push_back(MonsterCard("The Delivery", 3, 1, 3, {
            { std::make_tuple("Wilbur & Chick", "Docks", "Barn") } }, { "Dracula" }));
        monsterDeck.push_back(MonsterCard("Fortune Teller", 3, 1, 2, {
            { std::make_tuple("Maleva", "Camp", "Institute") } }, { "Dracula" }));
        monsterDeck.push_back(MonsterCard("Former Employer", 3, 1, 2, {
            { std::make_tuple("Dr. Cranly", "Laboratory", "Precinct") } }, { "Invisible Man", "Dracula" }));
        monsterDeck.push_back(MonsterCard("The Innocent", 3, 1, 3, {
            { std::make_tuple("Maria", "Barn", "Inn") } }, { "Dracula", "Invisible Man" }));
        monsterDeck.push_back(MonsterCard("Hurried Assistant", 3, 2, 3, {
            { std::make_tuple("Fritz", "Tower", "Laboratory") } }, { "Dracula" }));
        monsterDeck.push_back(MonsterCard("Egyptian Expert", 3, 2, 2, {
            { std::make_tuple("Prof.Pearson", "Cave", "Museum") } }, { "Dracula", "Invisible Man" }));
        monsterDeck.push_back(MonsterCard("The Ichthyologist", 3, 1, 2, {
            { std::make_tuple("Dr.Read", "Institute", "Hospital") } }, { "Invisible Man" }));
        
        // Colored cards
        for (int i = 0; i < 3; ++i) {
            monsterDeck.push_back(MonsterCard("Form of The Bat", 2, 1, 2, {
                { std::make_tuple("DraculaToHero", "", "") } }, { "Invisible Man" }));
        }
        for (int i = 0; i < 3; ++i) {
            monsterDeck.push_back(MonsterCard("Sunrise", 0, 1, 2, {
                { std::make_tuple("DraculaToCrypt", "", "") } }, { "Invisible Man", "Dracula" }));
        }
        for (int i = 0; i < 5; ++i) {
            monsterDeck.push_back(MonsterCard("Thief", 2, 1, 3, {
                { std::make_tuple("InvisibleToMaxItems", "", "") } }, { "Invisible Man", "Dracula" }));
        }
        
        // New Bonus Cards
        for (int i = 0; i < 2; ++i) {
            monsterDeck.push_back(MonsterCard("Hypnotic Gaze", 2, 1, 2, {
                { std::make_tuple("NearestToDracula", "", "") } }, { "Invisible Man" }));
        }
        for (int i = 0; i < 2; ++i) {
            monsterDeck.push_back(MonsterCard("On The Move", 3, 3, 2, {
                { std::make_tuple("VillagersToSafety", "", "") } }, { "Frenzied Monster" }));
        }
    ...
    void Game::initializeFrenzySystem() {
        // Dracula has Frenzy Order 1, Invisible Man has Frenzy Order 6
        // At start, Dracula gets the Frenzy Marker (lowest order)
    ...
    void Game::passFrenzyMarker() {
        // Pass frenzy marker to the next monster with lowest Frenzy Order
        // Dracula (Order 1) -> Invisible Man (Order 6)
    ...
    bool Game::isGameWon() {
        ...
        if (draculaDefeated && invisibleManDefeated) {
           ...
           // showFinalStatus(); // Removed from here
            return true;
        }
        return false;
    }
    ...
    //         if (loc->getName() == name) return loc;
    //     }
    ...
    void Game::advanceTask(Hero* hero) {
        ...
        // === Drop Evidence برای Invisible Man ===
        for (Monster* monster : monsters) {
    ...
    void Game::defeatMonster(Hero* hero) {
        ...
        for (Monster* monster : monsters) {
            if (!monster->isDefeated() && monster->getLocation() == heroLoc) {
                ...
                // بررسی شرایط defeat با توجه به آیتم‌ها
                if (monster->canBeDefeated(hero->getItems())) {
                    monster->defeat();
                    // Remove monster from its location upon defeat
                    ...
                    if (monster->getName() == "Dracula") draculaDefeated = true;
                    if (monster->getName() == "Invisible Man") invisibleManDefeated = true;
                    
                    // Pass frenzy marker when a monster is defeated
        ...
        std::cout << "⚠️ No monster to defeat at your location.\n";
    }
    ...
    void Game::monsterPhase() {
        ...
        if (!monsterDeck.empty()) {
            ...
            if (card.getName() == "Hypnotic Gaze") {
                ...
                // Pass frenzy marker and move villagers towards safety
                passFrenzyMarker();
                
                // Move each villager one step closer to their destination
                for (auto villager : villagers) {
                    if (villager && !villager->getIsSaved()) {
            ...
            for (const std::string& monsterName : card.getMonsterMovementOrder()) {
                ...
                if (!monster || monster->isDefeated()) continue;
                ...
                for (int i = 0; i < card.getDiceRolls(); ++i) {
                    ...
                    // Only attack if starResult == '*' and there is a target
                    if (starResult == "*") {
                        bool attackFinished = monster->attack(this, starResult, exResult);
                        if (attackFinished) {
                            // Ensure all attack consequences/messages are processed before ending phase
                            // (No break here, let attack() finish first)
                            attackOccurred = true;
                            break; // End dice rolling for this monster after attack
                        }
                    } else if (exResult == "!") {
                        monster->performAction(this);
                    }
                }
                if (attackOccurred) break; // End monster phase after the attack is fully processed
            }
        } else {
...
//                     << " | Status: " << (monster->isDefeated() ? "Failed " : "active❗") << '\n';
//         }


//         // نمایش وضعیت تابوت‌های دراکولا
...
//         // وضعیت Villagerها
//         int savedCount = 0, aliveCount = 0;
//         for (auto villager : game.getVillagers()) {
//             if (villager->getIsSaved()) savedCount++;
//             else aliveCount++;
//         }
//         std::cout << "\nVillager Status:\n";
//         std::cout << "Rescued: " << savedCount << "\n";
//         std::cout << "In danger: " << aliveCount << "\n";

//         // آیتم‌های مکان فعلی قهرمان‌ها
//         std::cout << "\nCurrent location items:\n";
//         Location* loc1 = hero1->getLocation();
//         Location* loc2 = hero2->getLocation();
//         std::cout << hero1->getPlayerName() << " → " << loc1->getItems().size()
//                 << " Item in" << loc1->getName() << '\n';
//         std::cout << hero2->getPlayerName() << " → " << loc2->getItems().size()
//                 << " Item in" << loc2->getName() << '\n';

//                 const std::vector<std::string>& rolls = game.getLastMonsterDiceRolls();
//                 std::cout << "\n🎲 Last Monster Dice Rolls:\n";
//                 if (rolls.empty()) {
//                     std::cout << "No monster rolled dice yet.\n";
//                 } else {
//                     for (const std::string& roll : rolls) {
...
void displayGameStatus(Game& game, Hero* hero1, Hero* hero2) {
    ...
    for (Hero* hero : {hero1, hero2}) {
        ...
        for (const auto& perk : perks) std::cout << perk.getName() << " ";
        std::cout << "\n";
    }
    ...
    for (auto loc : game.getLocations()) {
        ...
        if (!hasSomething) std::cout << "(empty)";
        std::cout << "\n";
    }
    ...
    Location* Game::findNextLocationTowards(Location* from, Location* to) {
        ...
        std::set<Location*> visited;
        std::queue<std::pair<Location*, Location*>> q; // {current, previous}
        q.push({from, nullptr});
        visited.insert(from);
        ...
        while (!q.empty()) {
            ...
            if (current == to) {
                // Found target, backtrack to find the first step
                while (previous != from) {
                    current = previous;
                    // Find the previous location in the path
